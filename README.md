...
Not yet published to npm; install from the repository until the first release.
...
Every tool takes `agent`, the alias from the configuration, as its first argument.
...
Read the score with its cause: on the `2026-07-28` run, 32 of the 37 scored scenarios fail at least one check, 19 because they call tool names written into the suite (conformance issue 248) and 13 because they exercise prompts, resources, completions or logging, which a bridge to A2A agents does not declare. All 72 failing checks are in the committed baseline, each with its reason, the suite exits 0 on both revisions, and the protocol checks that apply to a bridge are green: wire schema, `server/discover`, the stateless wire, SEP-2243 headers, DNS rebinding protection. The numbers are in [`conformance/REPORT.md`](conformance/REPORT.md), regenerated by the command below.
